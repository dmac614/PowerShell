...
$DeviceUptime = (Get-CimInstance -ClassName win32_operatingsystem).LastBootUpTime
    ...
    if ($DeviceUptime.Timeofday.TotalHours -gt 24) { 
        ...
        Write-Output "Restarting $([Environment]::MachineName) in 15 seconds`nAfter the restart, attempt to print again"
        System.Diagnostics.Process.Start("shutdown", "/r /t 15")
